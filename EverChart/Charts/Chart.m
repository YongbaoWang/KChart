...
-(void)initBtn{
    ...
    sec1BtnRect = CGRectZero;
    sec2BtnRect = CGRectZero;
    ...
    NSDictionary *attributes = @{NSFontAttributeName:[UIFont systemFontOfSize:12],NSForegroundColorAttributeName:[UIColor whiteColor],NSParagraphStyleAttributeName:style};
    CGSize btnSize = [@"日" sizeWithAttributes:attributes];
    
    NSArray *btnTitleArray1 = @[@"日",@"周",@"月"];
    NSArray *btnTitleArray2 = @[@"成交量",@"RSI",@"KDJ",@"WR",@"VR"];
    
    Section *sec1 = self.sections[0];
    CGRect btn1Rect = CGRectMake(self.paddingLeft, (self.paddingTop + sec1.paddingTop)/2.0, sec1.paddingLeft - 5, (self.paddingTop + sec1.paddingTop)/2.0);
    
    UIBezierPath *kPath = [UIBezierPath bezierPathWithRoundedRect:btn1Rect cornerRadius:4];
    CGContextBeginPath(context);
    CGContextAddPath(context, kPath.CGPath);
    CGContextClosePath(context);
    CGContextFillPath(context);
    
    [btnTitleArray1[self.candleIndex] drawInRect:CGRectInset(btn1Rect, 0, (btn1Rect.size.height - btnSize.height)/2.0) withAttributes:attributes];
    ...
    CGRect btn2Rect = CGRectMake(self.paddingLeft,sec2.frame.origin.y , sec1.paddingLeft + 5, sec2.paddingTop);
    CGContextBeginPath(context);

    UIBezierPath *path2 = [UIBezierPath bezierPathWithRoundedRect:btn2Rect cornerRadius:4];
    CGContextAddPath(context, path2.CGPath);
    CGContextClosePath(context);
    CGContextFillPath(context);
    
    [btnTitleArray2[sec2.selectedIndex] drawInRect:CGRectInset(btn2Rect, 0, (btn2Rect.size.height - btnSize.height)/2.0) withAttributes:attributes];

    sec1BtnRect = btn1Rect;
    sec2BtnRect = btn2Rect;
...
-(void)drawChart{
    ...
    for(int secIndex=0;secIndex<self.sections.count;secIndex++){
        ...
        //防止新上市股票，总数据条数较少时，单个宽度过大
        if (plotWidth > 15 && self.plotCount < self.range) {
            plotWidth = 15;
        }
    ...
    for(int i=0;i<self.sections.count;i++){
		...
		for(int s=0;s<sec.series.count;s++){
			...
			for(int j=0;j<label.count;j++){
                ...
                UIColor *textColor = [UIColor colorWithRed:[[colors objectAtIndex:0] floatValue] green:[[colors objectAtIndex:1] floatValue] blue:[[colors objectAtIndex:2] floatValue] alpha:1];
                
                CGSize textSize = [text sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14]}];
                CGRect rect = CGRectZero;
                if (i == 0 && s == 0) {
                    rect = CGRectMake(sec.frame.origin.x + w, sec.frame.origin.y + (sec.paddingTop/2 - textSize.height)/2, textSize.width, textSize.height);
                }else if(i == 0 && s > 0){
                    rect = CGRectMake(sec.frame.origin.x + sec.paddingLeft + w, sec.frame.origin.y + sec.paddingTop/2 + (sec.paddingTop/2 - textSize.height)/2, textSize.width, textSize.height);
                }
                else {
                ...
                [text drawInRect:rect withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14],NSForegroundColorAttributeName:textColor}];
                w += textSize.width + 5;
			}
		}
	}
...
//	CGContextSetRGBFillColor(context, 0.5, 0.5, 0.5, 1.0);
	...
	for(int secIndex=0;secIndex<self.sections.count;secIndex++){
		...
		for(int aIndex=0;aIndex<sec.yAxises.count;aIndex++){
            ...
            //原点处的Y轴刻度
            if ([yAxisType isEqualToString:@"candle"]) {
                [[@"" stringByAppendingFormat:format,yaxis.baseValue] drawInRect:CGRectMake(0, baseY - kYFontSize, sec.frame.origin.x + sec.paddingLeft, kYFontSize * 2) withAttributes:@{NSFontAttributeName:[UIFont fontWithName:KYFontName size:kYFontSize],NSParagraphStyleAttributeName:style,NSForegroundColorAttributeName:kYFontColor}];
            }
            ...
            if (yaxis.max < yaxis.min) {
                NSLog(@"Y轴刻度错误，yaxis.max < yaxis.min");
                return;
            }
...
//                    }
                    ...
                    //Y轴最大值，显示位置靠下
                    CGFloat offset = (i == yaxis.tickInterval) ? 0 : kYFontSize/2 ;
                
                    [valueY drawInRect:CGRectMake(0, iy - offset, sec.frame.origin.x + sec.paddingLeft, kYFontSize * 2) withAttributes:@{NSFontAttributeName:[UIFont fontWithName:KYFontName size:kYFontSize],NSParagraphStyleAttributeName:style,NSForegroundColorAttributeName:kYFontColor}];
			...
			for(int i=1; i <= yaxis.tickInterval;i++){
				if(yaxis.baseValue - i*step >= yaxis.min && yaxis.min < MAXFLOAT){
                    ...
                    if (i == yaxis.tickInterval + 1) {
                        if (![yAxisType isEqualToString:@"candle"]) {
                            valueY = @"";
                        }
                    }
                    [valueY drawInRect:CGRectMake(0, iy - 7, sec.frame.origin.x + sec.paddingLeft, kYFontSize * 2) withAttributes:@{NSFontAttributeName:[UIFont fontWithName:KYFontName size:kYFontSize],NSParagraphStyleAttributeName:style,NSForegroundColorAttributeName:kYFontColor}];
		...
		if(self.touchFlag == 0){
            ...
            //双指同时向左滑动
				...
				if(fabs(fabs(currFlagTwo-currFlag)-fabs(self.touchFlagTwo-self.touchFlag)) >= MIN_INTERVAL){
                    //放大
					if(fabs(currFlagTwo-currFlag)-fabs(self.touchFlagTwo-self.touchFlag) > 0){
                        
                        if (self.rangeTo - self.rangeFrom > 30) { //防止过度放大
                            if(self.plotCount>self.rangeTo-self.rangeFrom){
                            ...
                            NSLog(@"da：from:%d,to:%d",self.rangeFrom,self.rangeTo);
                            [self setNeedsDisplay];
